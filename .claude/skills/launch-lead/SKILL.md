...
>    **GOVERNANCE (owner MANDATORY, doc `3e04822a`) — every dokan script I create/own:** (a) **`upload_script(created_by:'launch-lead')`** + own the catalogue `9356bddc` row (no anonymous/orphan scripts; upsert no-ops on identical source, so a metadata-only owner fix needs a trivial source change to write); (b) **self-send a P0 relay receipt `ttl_seconds:86400`** — subject `DOKAN RECEIPT — <name> (<id>)`, body = the dokan response verbatim (id/name/schedule/what-it-does); (c) **zero local scripts** — everything lives in dokan; a recurring local `.mjs/.sh/.py` → migrate it; a genuinely host-bound one (can't run in a container, e.g. `launch-lint.py` reads the host SQLite store) → **flag cmo, never silent**.
