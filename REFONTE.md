...
trovex passe de *« router token-efficient sur des .md in-tree »* à **un outil de centralisation de doc où trovex *est* la doc**.
...
| `/write` | agents | l'agent émet le contenu **une fois** ; trovex stocke. SSOT par construction (instance unique partagée) |
| **frontend** | humains | browse / lecture / recherche via l'UI web — aucun fichier local requis |
| `.trovexignore` | — | la **seule** raison de garder un vrai fichier sur disque : un **consommateur file-native** (build de doc, outil du repo qui lit les .md, humain qui veut son éditeur). Tout le reste vit **uniquement dans trovex** |
...
| Régénérer un report introuvable | il est dans trovex → **trouvable**, pas refait |
...
## 6. Backlog plomberie (interne à trovex, invisible du contrat)
...
- **Relay** : dégraissage → transport pur ; vault + mémoire migrent vers trovex (rupture de contrat `search_vault` / `get_memory` → phase de compat).
...
**Livré dans ce repo :**
- ✅ `trovex_write` / `trovex_read` — contenu, pas pointeurs (v0.9)
- ✅ Store pluggable `SqliteStore` (Pôle A), docs adressés par ID opaque → switch Supabase = drop-in (v0.9)
- ✅ docs trovex sous `source_id='trovex'` virtuel, jamais purgés par l'indexeur (v0.9)
- ✅ `kind='record'` exempté de péremption-âge + dédup (v0.9)
- ✅ Extraction de **section** au `trovex_read` (servir le minimum) (v0.10)
- ✅ **Sérialisation des écritures** (lock côté serveur) (v0.10)
- ✅ **Hook d'enforcement** `deploy/hooks/trovex-md-guard.sh` + `.trovexignore.example` (deny .md direct → `trovex_write`, dégradation gracieuse si trovex down) (v0.10)
...
- ⏸️ **Supabase (Pôle B)** — infra post-dogfood ; le `Store` est déjà l'interface de swap, rien à faire côté trovex tant que le besoin n'est pas là.
- ⏸️ **Dégraissage de relay + migration `search_vault`/`get_memory`** — c'est le repo **`agent-relay`**, un autre système. À mener là-bas, pas dans trovex.
