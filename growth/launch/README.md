# growth/launch — index + fire order

**Status:** index of the launch/distribution kit. **Everything here is a DRAFT** — nothing is posted or
submitted live; a human (owner) fires each. launch-lead drafts; the owner pulls the trigger.
**Owner:** launch-lead · **Maintained against:** the assets in this folder + `../agency/launch/`

> One map so the owner/cmo can find every asset and fire in the right order. The governing sequence is
> `launch-day-runbook.md`; the evidence behind it is `launch-teardown-playbook.md`. Phase: private beta now
> (north-star = **waitlist signups**); coordinated public OSS launch later (cmo signals go).
...
**Show HN / Product Hunt (one-shots)**
...
- `press-onepager.md` — press fact sheet for tsukumo + the suite + asset list.
...
## Beta phase (now) — waitlist drive

Private beta is live; public channels stay HOLD. The active push:
- `beta-reframe.md` — waitlist drivers + the public-launch HOLD index (what's frozen vs live).
- `beta-sequence.md` — private-beta launch sequence (waitlist drive).
- `beta-outreach.md` — targeted "request access" community outreach.
- `beta-onboarding.md` — approved-tester welcome + onboarding.
- `proof-plan.md` — capture real beta proof (numbers + permissioned quotes) — gates the public flip.
- `public-launch-kit-deep.md` — deepened public-launch kit, **FROZEN** until the hold lifts.

---

## Fire order (the short version — full detail in `launch-day-runbook.md`)

1. **Now (beta):** drive waitlist (`beta-*`), capture proof (`proof-plan.md`). Public channels HOLD.
2. **Pre-launch (compounding, reversible):** registries + free directories (`mcp-registries`, `directory-*`) once repo public + on PyPI (`unfreeze-checklist`). wrai.th can seed ahead (it's public).
3. **Coordinated launch day (cmo signals go, gate green):** wrai.th Show HN (lead one-shot) → Product Hunt (separate day) → community + earned media (spaced, value-first). trovex flips public when its beta proof + repo are ready.
4. **Post:** hand metrics to analytics-lead; watch the **consulting-lead door** (north star), not vanity rank.

**Owner go-gates** (the only blockers, consolidated): tsukumo.ch live · trovex waitlist working · deploy verified · design assets ready · cmo go + owner free 2–3h · platform account creds · beta proof for any proof claim. See `launch-day-runbook.md §7`.

---

## Rules that apply to every asset here

- **Drafts only.** launch-lead never submits/posts; the owner fires.
- Developer-honest, plain, cost-framed. Lowercase wordmarks (`trovex`, `wrai.th`, `yoru`, `tsukumo`).
- Real **~60%** only (repo-dependent, verifiable); no fabricated proof/quotes/users; **no public prices** for the agency.
- No "Synergix" in prose (only the GitHub-org/URL identifier). No vote-begging, no booster comments, no spam.
- Consulting stays a low-key footnote on OSS surfaces — never a pitch.

*Index of drafts. Nothing here has been fired.*
