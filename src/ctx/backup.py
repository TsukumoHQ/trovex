"""Online SQLite backups for the ctx store (now the sole corpus).
...
def make_backup(db_path: Path, data_dir: Path) -> Path:
    ...
    try:
        ...
        flush.close()
    dest = bdir / f"ctx-{time.strftime('%Y%m%d-%H%M%S')}.db"
...
def list_backups(data_dir: Path) -> list[dict]:
    ...
    if not bdir.exists():
        return []
    out = []
    for p in sorted(bdir.glob("ctx-*.db"), reverse=True):
        st = p.stat()
...
def prune(data_dir: Path, keep: int = KEEP) -> int:
    backups = sorted(backup_dir(data_dir).glob("ctx-*.db"), reverse=True)
    removed = 0
