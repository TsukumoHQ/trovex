...
Without ctx, an agent typically does Glob/Grep then Read on 2-3 .md files
to triage. With ctx(), they get a ranked list with status markers and
...
`response_tokens_est` = the ctx() output size (~50-150 tokens)
"""
...
class SavingsRow:
    ...
    response_tokens: int        # ∑ ctx output tokens
    actual_read: int            # ∑ top-1 result tokens (what they likely Read after ctx)
