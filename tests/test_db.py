...
class _BoomOnScript(sqlite3.Connection):
    """Connection subclass whose executescript() can be armed to raise --
    simulates a process killed partway through a migration's destructive
    executescript() (used only where a plain sqlite3.Connection instance
    doesn't allow attribute assignment for a simpler monkeypatch)."""
...
def _boom_vec_conn(tmp_path, name="raw.db"):
    conn = sqlite3.connect(str(tmp_path / name), factory=_BoomOnScript)
    conn.row_factory = sqlite3.Row
    conn.enable_load_extension(True)
    sqlite_vec.load(conn)
    conn.enable_load_extension(False)
...
def test_interrupted_partition_migration_is_atomic_and_self_heals(tmp_path):
    ...
    # the old vec_docs has already been dropped.
    conn.boom = True
